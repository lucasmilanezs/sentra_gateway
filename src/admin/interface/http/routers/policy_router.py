...
async def get_policy(route_id: str, claims: Annotated[JwtClaims, Depends(require_permission("routes"))], uc: Annotated[ManagePolicy, Depends(get_manage_policy)]):
    return await uc.get_by_route(caller_role=claims.role, caller_tenant_id=claims.tenant_id, route_id=route_id)

@router.put("/{route_id}/policy", response_model=PolicyResponse, status_code=status.HTTP_200_OK)
async def upsert_policy(route_id: str, body: PolicyUpsert, claims: Annotated[JwtClaims, Depends(require_permission("routes"))], uc: Annotated[ManagePolicy, Depends(get_manage_policy)]):
    return await uc.upsert(
        caller_role=claims.role, caller_tenant_id=claims.tenant_id, route_id=route_id,
        requires_auth=body.requires_auth, rate_limit_per_minute=body.rate_limit_per_minute,
        allowed_roles=body.allowed_roles, jwt_validate_exp=body.jwt_validate_exp,
        jwt_issuer=body.jwt_issuer, jwt_audience=body.jwt_audience,
