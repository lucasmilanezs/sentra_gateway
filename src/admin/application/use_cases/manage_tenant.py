...
class ManageTenant:
    ...
    async def create(self, name: str, alias: str, *, caller_user_id: str | None = None, caller_role: str = "superuser") -> Tenant:
        alias_n = Tenant.normalize_alias(alias)
    ...
    async def update(
        ...
        *,
        ...
        new_name = Tenant.normalize_name(data["name"]) if "name" in data else t.name
        new_alias = Tenant.normalize_alias(data["alias"]) if "alias" in data else t.alias
        if new_alias != t.alias:
            existing = await self._tenants.get_by_alias(new_alias)
            if existing and existing.id != tenant_id:
                raise ConflictError("alias já em uso")
        updated = t.with_updates(name=new_name, alias=new_alias, now=_utcnow())
